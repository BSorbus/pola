class WorksController < ApplicationController
  ...
  def datatables_index_trackable
    respond_to do |format|
      format.json{ render json: WorkDatatable.new(view_context, { trackable_id: params[:trackable_id], trackable_type: params[:trackable_type] }) }
    end
  end

  def datatables_index_user
    respond_to do |format|
      format.json{ render json: WorkDatatable.new(view_context, { only_for_current_user_id: params[:user_id] }) }
    end
  end
